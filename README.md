...
| 私有知识库 | `paper-qa` | 本地 PDF 建库、带引用 RAG（**占位**·需配 API key） | 封装 paper-qa |
...
> **占位**技能（`survey-builder` / `paper-qa`）已装好说明与兜底流程，但要完全可用还需补一步（换源 / 配 API key），详见各自 `SKILL.md`。
...
- **可选依赖（需 key / 易被墙）**：`fund-literature-review-writer` 的 `scholarly`（Google Scholar，限流）/ `exa-py`（需 `EXA_API_KEY`）；`paper-qa` 默认不装，用时再 `pip install "paper-qa>=5"`（需 `OPENAI_API_KEY` 或 LiteLLM 配置）。
