...
本目录是一组**独立的科研 / 医学写作技能**（20 个），**没有顶层主控 / 路由**——每个技能靠自己 `<名>/SKILL.md` 里的 `description` 自行触发，agent 命中哪个用哪个。仓库根的 `AGENTS.md` / `CLAUDE.md` 只是项目说明与几条通用硬规矩，不是调度器。
...
- **演示制作**：`ppt-master`（文档→原生可编辑 PPTX，SVG 多角色流水线）
