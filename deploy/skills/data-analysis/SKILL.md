---
name: data-analysis
description: 用于任何数据分析、统计计算、画图、读写 CSV/Excel 的任务。使用容器内的 Python（内含 pandas / numpy / scipy / matplotlib / scikit-learn / seaborn / statsmodels）。当用户上传数据文件或要求分析、统计、探索性可视化时使用。边界：**探索性看数与 150dpi 预览图**用本技能；要**投稿级出版图**（森林图/KM/火山图，300dpi+矢量）用 nature-figure。
---

# 数据分析技能（服务器 / 容器版）

> **本仓库运行环境（先读）**：Python 直接用 `python3`（容器镜像已装好科学计算包）；产出写 `outputs/`（有会话专属目录 `outputs/<会话id>/` 时以它为准、勿写仓库根固定名——多用户共享会 clobber）。

容器镜像已装好科学计算包。**运行 Python 用 `python3`**。命中本技能就**直接做，别回绕**。

## 解释器
```
python3   # 容器 / 服务器（镜像内已装依赖）
```
...
- **Meta 分析（定量合并）**：用 `statsmodels.stats.meta_analysis`（`combine_effects`：DerSimonian-Laird 随机效应、I²/τ²/Q 异质性）+ 自绘森林图；亚组/敏感性分析。⚠️ **Egger 发表偏倚检验/漏斗图不对称、REML、网络 Meta/多水平 statsmodels 无内置**——需手写加权回归或用 R `metafor`；**别声称能做其实做不了的**。定量合并（森林图/异质性）可在本技能内做；筛选 / RoB / GRADE / PRISMA 属系统综述方法学，本套件无独立技能，叙述性综述见 `survey-builder`。
...
> **基线特征表 Table 1 与样本量/把握度**：本技能即可完成（按变量类型自动选检验、连续变量报 `均数±SD`/`中位数[IQR]`、分类变量报 `n (%)`）；稿件成文后的**统计报告审查**（检验选择/多重比较/图注规范）另走 `nature-statistics` 技能。

## 数据治理（可复现的底线，动分析前先立规矩）
- **原始数据只读**：原始文件**绝不就地修改**；清洗/派生、分析产物一律写到**本会话产物目录**（有会话专属目录 `outputs/<会话id>/` 时用它，无则仓库根 `outputs/`；下称 `<产物目录>`，可再分 `<产物目录>/processed/`、`<产物目录>/analysis/`）。原始数据进 `.gitignore`（含患者信息更要）。
...
- **所有产出（图表 PNG、结果 CSV/Excel）写到 `<产物目录>`**（有会话专属目录 `outputs/<会话id>/` 时用它，无则 `outputs/`），方便前端用户下载；**别写仓库根固定名**（多用户共享会 clobber）。
- 画图用无界面后端：脚本开头 `import matplotlib; matplotlib.use("Agg")`，再 `plt.savefig("<产物目录>/xxx.png", dpi=150, bbox_inches="tight")`（把 `<产物目录>` 换成真实路径，如 `outputs/<会话id>/`）。**若产物要嵌进 md/论文，嵌入路径必须与此落盘路径逐字一致。**
