...
> **本仓库运行环境（先读）**：Python 用 `.venv/Scripts/python.exe`（Windows）/ `.venv/bin/python`（项目根 `.venv`；缺则先跑 `env-setup`）。脚本在 `skills/ppt-master/scripts/`、模板与图标库在 `skills/ppt-master/templates/`（图标按名检索：`ls templates/icons/<库>/ | grep <关键词>`）。核心 PPTX 依赖（python-pptx / svglib / reportlab / mammoth / markdownify 等）**已随一键脚本装进 `.venv`，无需再 pip install**；AI 配图（google-genai / Gemini）、旁白音频（edge-tts）、SVG 编辑器（flask）为**可选**功能，需自备 key 或按需启用。**产物一律写 `outputs/`（有会话专属目录时以它为准，勿写仓库根或技能目录）**。注：上游 44M 图片风格对比库 `references/ai-image-comparison/` 在本仓库已删除（无界面文本 Agent 用不上；`strategist.md` 里相关链接会失效，无碍）。

> **【本平台交互约定 · 覆盖下文任何“弹选项卡 / 结构化提问 / AskUserQuestion”的说法】**
> 需要用户在多个方向间拍板时（含本技能各 ⛔ BLOCKING 确认闸），**禁止**调用 `AskUserQuestion`、也**不要**输出前端会渲染成选项卡的 `ask` JSON 代码块或任何弹窗式多选控件。一律改为在**正文用纯文本编号候选**：列 2–4 个具体、互斥的选项，每个一句话点明差异与代价；**把推荐项放第 1 个并写明“推荐 1，因为……”**；结尾明确告诉用户**直接回一个数字（如 1 / 2 / 3）即可推进**。用户只回数字＝选定该项、直接进入下一步、不再追问确认；回了别的（文字 / 否定 / 多选）按其本意走。仅当信息根本无法枚举（纯事实补录）时才用开放式提问。
